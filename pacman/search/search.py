...
# Pieter Abbeel (pabbeel@cs.bedepthFirstSearchrkeley.edu).
    ...
    while not problem.isGoalState(state) and not states_stack.isEmpty():
        ...
        for (state, last_action, stepCost) in successor:
            if state not in visited_states:
                states_stack.push((state, actions + [last_action]))
...
def breadthFirstSearch(problem):
    ...
    # Initialize
    state = problem.getStartState()
    states_queue.push((state, actions))
    last_action = None

    while (not problem.isGoalState(state)) and (not states_queue.isEmpty()):
        state, actions = states_queue.pop()
        successor = problem.getSuccessors(state)
...
def uniformCostSearch(problem):
    ...
    # Initialize
    state = problem.getStartState()
    states_queue.push((state, actions, stepCost), stepCost)
    last_action = None

    while (not problem.isGoalState(state)) and (not states_queue.isEmpty()):
        state, actions, last_stepCost = states_queue.pop()
        successor = problem.getSuccessors(state)
        for (next_state, last_action, stepCost) in successor:
            if next_state not in visited_states:
                visited_states.add(next_state)
